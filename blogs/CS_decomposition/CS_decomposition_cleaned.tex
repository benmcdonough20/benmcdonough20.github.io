...
\begin{align}
    U^\dagger M V &= \begin{pmatrix}C \\ S\end{pmatrix}
\end{align}
...
\begin{proof}
Let $M = \begin{pmatrix}X \\ Y\end{pmatrix}$. Choose unitaries $U_1, V$ such that $U_1^\dagger X V = C$, where $C = \diag(c_1, \dots, c_k)$. Since $X$ is a submatrix of $M$ whose singular values are all 1, we have $c_i \leq 1$. Then
$$
\begin{pmatrix}U_1^\dagger & 0 \\ 0 & 1\end{pmatrix}\begin{pmatrix}X \\ Y\end{pmatrix}V = \begin{pmatrix}U_1^\dagger X V \\ YV\end{pmatrix} = \begin{pmatrix}C \\ YV\end{pmatrix}
$$
...
$$
\begin{pmatrix}U_1^\dagger & 0 \\ 0 & Q^\dagger\end{pmatrix}\begin{pmatrix}X \\ Y\end{pmatrix}V = \begin{pmatrix}U_1^\dagger X V \\ QYV\end{pmatrix} = \begin{pmatrix}C \\ S\end{pmatrix}
$$
...
$$
U^\dagger M V = \begin{pmatrix}C & -S \\ S & C\end{pmatrix}
$$
...
\begin{proof}
    ...
    $$
    \begin{pmatrix}U_1^\dagger & 0 \\ 0 & 1\end{pmatrix}\begin{pmatrix}X & Y \\ Z & W\end{pmatrix} \begin{pmatrix}V_1 & 0 \\ 0 & 1\end{pmatrix} = \begin{pmatrix}U_1^\dagger X V_1 & U_1^\dagger Y \\ ZV_1 & W\end{pmatrix} = \begin{pmatrix}C & Y' \\ Z' & W\end{pmatrix}
    $$
    ...
    $$
    \begin{pmatrix}U_1^\dagger & 0 \\ 0 & L^\dagger\end{pmatrix}\begin{pmatrix}X & Y \\ Z & W\end{pmatrix} \begin{pmatrix}V_1 & 0 \\ 0 & R\end{pmatrix} = \begin{pmatrix}U_1^\dagger X V_1 & U^\dagger_1YR \\  L^\dagger ZV_1 & L^\dagger WR\end{pmatrix} = \begin{pmatrix}C & S \\ -S & W'\end{pmatrix}
    $$
    Writing $C = 1 \oplus C'$, where $C'$ consists of $c_i$ strictly less than 1, then we have
$$
    \begin{pmatrix}C & S \\ -S & W'\end{pmatrix} = \begin{pmatrix}\mqty{1 & 0 \\ 0 & C'}  & \mqty{0 & 0 \\ 0 & S'} \\ \mqty{0 & 0 \\ 0 & -S'} & \mqty{X_1 & X_2 \\ X_3 & X_4}\end{pmatrix}
$$
...
$$
    \begin{pmatrix}\mqty{1 & 0 \\ 0 & C'}  & \mqty{0 & 0 \\ 0 & S'} \\ \mqty{0 & 0 \\ 0 & -S'} & \mqty{X_1 & X_2 \\ X_3 & X_4}\end{pmatrix} = \begin{pmatrix}\mqty{1 & 0 \\ 0 & C'}  & \mqty{0 & 0 \\ 0 & S'} \\ \mqty{0 & 0 \\ 0 & -S'} & \mqty{1 & 0 \\ 0 & C'}\end{pmatrix} =\begin{pmatrix}C & -S \\ S & C\end{pmatrix}
$$
...
\begin{align}
    P_F &= \begin{pmatrix}P_0 & 0 & \dots & 0 \\ 0 & P_0 & \dots & 0 \\ 0 & 0 & \dots & P_0\end{pmatrix} \\
    P_G &= \begin{pmatrix}P(\theta_1) & 0 & \dots & 0\\ 0 & P(\theta_2) & \dots & 0 \\ 0 & 0 & \dots & P(\theta_k)\end{pmatrix}
\end{align}
\end{thm}
\begin{proof}
...
$$
U^\dagger M V = \begin{pmatrix}U_1^\dagger & 0 \\ 0 & U_2^\dagger\end{pmatrix}\begin{pmatrix}X^\dagger Y \\ X_\perp^\dagger Y\end{pmatrix}V = \begin{pmatrix}U_1^\dagger X^\dagger YV \\ U_2^\dagger X_\perp^\dagger YV\end{pmatrix} = \begin{pmatrix}C \\ S\end{pmatrix}
$$
Since $P_F = XX^\dagger$ and $P_G = YY^\dagger$, putting $U = (XU_1, X_{\perp} U_2)$, we have 
$$
U^\dagger P_F U = \begin{pmatrix}U_1^\dagger & 0 \\ 0 & U_2^\dagger\end{pmatrix}\begin{pmatrix}1 & 0 \\ 0 & 0\end{pmatrix} \begin{pmatrix}U_1^\dagger & 0 \\ 0 & U_2^\dagger\end{pmatrix} = \begin{pmatrix}1 & 0 \\ 0 & 0\end{pmatrix}
$$
$$
U^\dagger P_G U = (U^\dagger Y V)(V^\dagger Y^\dagger U) = \begin{pmatrix}C \\ S\end{pmatrix} \begin{pmatrix}C & S\end{pmatrix} = \begin{pmatrix}C^2 & CS \\ CS & S^2\end{pmatrix}
$$
...
$$
    P_F = \frac{1-s}{2} = \begin{pmatrix}1 & 0 \\ 0 & 0\end{pmatrix}
$$
$$
    P_G = \frac{1-sr}{2} = \begin{pmatrix}\cos^{2}(\lambda/2) & \cos(\lambda/2)\sin(\lambda/2) \\ \cos(\lambda/2)\sin(\lambda/2) & \sin^{2}(\lambda/2)\end{pmatrix}
$$
...
\begin{proof}
    Consider
    $$
    P_F^\dagger P_G \sim \begin{pmatrix}1 & 0 \\ 0 & 0\end{pmatrix} \begin{pmatrix}C^2 & CS \\ CS & S^2\end{pmatrix} = \begin{pmatrix}C^2 & CS \\ 0 & 0\end{pmatrix}
    $$
    Then the singular values of the above matrix are the square roots of the eigenvalues of
    $$
    \begin{pmatrix}C^2 & CS \\ 0 & 0\end{pmatrix} \begin{pmatrix}C^2 & CS \\ 0 & 0\end{pmatrix}^\dagger = \begin{pmatrix}C^2(C^2+S^2) & 0 \\ 0 & 0\end{pmatrix} = \begin{pmatrix}C^2 & 0 \\ 0 & 0\end{pmatrix}
    $$
